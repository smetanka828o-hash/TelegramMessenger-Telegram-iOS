...
public class ChatTextInputPanelNode: ChatInputPanelNode, ASEditableTextNodeDelegate, ChatInputTextNodeDelegate {
    ...
    public let menuButton: LiquidGlassButtonNode
    ...
    public let attachmentButton: LiquidGlassButtonNode
    ...
    public let searchLayoutClearButton: LiquidGlassButtonNode
    ...
    public init(context: AccountContext, presentationInterfaceState: ChatPresentationInterfaceState, presentationContext: ChatPresentationContext?, presentController: @escaping (ViewController) -> Void) {
        ...
        self.menuButton = LiquidGlassButtonNode(config: LiquidGlassConfig())
        self.menuButton.clipsToBounds = true
        self.menuButton.updateCornerRadius(16.0)
        ...
        self.attachmentButton = LiquidGlassButtonNode(config: LiquidGlassConfig())
        ...
        self.searchLayoutClearButton = LiquidGlassButtonNode(config: LiquidGlassConfig())
        ...
        self.menuButton.pressed = { [weak self] in
            self?.menuButtonPressed()
        }
        self.menuButton.highligthedChanged = { [weak self] highlighted in
            if let strongSelf = self {
                strongSelf.menuButton.updateHighlight(highlighted)
                if highlighted {
                    strongSelf.menuButton.performPressDownAnimation()
                } else {
                    strongSelf.menuButton.performReleaseBounceAnimation()
                }
            }
        }
        ...
        self.attachmentButton.pressed = { [weak self] in
            self?.attachmentButtonPressed()
        }
        self.attachmentButton.highligthedChanged = { [weak self] highlighted in
            if let self {
                self.attachmentButton.updateHighlight(highlighted)
                if highlighted {
                    self.attachmentButton.performPressDownAnimation()
                } else {
                    self.attachmentButton.performReleaseBounceAnimation()
                }
            }
        }
...
//                    interfaceInteraction.finishMediaRecording(.dismiss)
        ...
        self.searchLayoutClearButton.highligthedChanged = { [weak self] highlighted in
            ...
            self.searchLayoutClearButton.updateHighlight(highlighted)
            if highlighted {
                self.searchLayoutClearButtonIcon.alpha = 0.6
                self.searchLayoutClearButton.performPressDownAnimation()
            } else {
                ...
                self.searchLayoutClearButton.performReleaseBounceAnimation()
            }
        }
        self.searchLayoutClearButton.pressed = { [weak self] in
            self?.searchLayoutClearButtonPressed()
        }
