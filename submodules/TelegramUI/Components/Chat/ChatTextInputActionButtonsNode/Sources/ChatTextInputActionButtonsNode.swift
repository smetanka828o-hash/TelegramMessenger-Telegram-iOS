...
public final class ChatTextInputActionButtonsNode: ASDisplayNode, ChatSendMessageActionSheetControllerSourceSendButtonNode {
    ...
    public let expandMediaInputButton: LiquidGlassButtonNode
    ...
    public init(context: AccountContext, presentationInterfaceState: ChatPresentationInterfaceState, presentationContext: ChatPresentationContext?, presentController: @escaping (ViewController) -> Void) {
        ...
        self.sendButton = LiquidGlassButtonNode(config: LiquidGlassConfig())
        
        self.textNode = ImmediateAnimatedCountLabelNode()
        self.textNode.isUserInteractionEnabled = false
        
        self.expandMediaInputButton = LiquidGlassButtonNode(config: LiquidGlassConfig())
        ...
        self.expandMediaInputButton.view.addSubview(self.expandMediaInputButtonBackgroundView)
        ...
        self.sendButton.highligthedChanged = { [weak self] highlighted in
            if let strongSelf = self {
                strongSelf.sendButton.updateHighlight(highlighted)
                if !strongSelf.sendButtonLongPressEnabled {
                    if highlighted {
                        strongSelf.sendButton.performPressDownAnimation()
                    } else {
                        strongSelf.sendButton.performReleaseBounceAnimation()
                    }
                } else {
                    if highlighted {
                        strongSelf.sendButton.performPressDownAnimation()
        ...
        self.expandMediaInputButton.highligthedChanged = { [weak self] highlighted in
            ...
            self.expandMediaInputButton.updateHighlight(highlighted)
            if highlighted {
                self.expandMediaInputButton.performPressDownAnimation()
            } else {
                self.expandMediaInputButton.performReleaseBounceAnimation()
            }
        }
    }
