...
public final class TabBarComponent: Component {
    ...
    public final class View: UIView, UITabBarDelegate, UIGestureRecognizerDelegate {
        ...
        private var lensViews: [AnyHashable: LiquidGlassView] = [:]
        private var pressedId: AnyHashable?
        ...
        @objc private func onTapGesture(_ recognizer: UITapGestureRecognizer) {
            ...
            case .began:
                if let id = closestItemId(to: point) {
                    self.pressedId = id
                    if let lens = self.lensViews[id] {
                        lens.performPressDownAnimation()
                        lens.isHighlighted = true
                    }
                }
            case .ended:
                if let id = closestItemId(to: point) {
                    if let lens = self.lensViews[id] {
                        lens.performReleaseBounceAnimation()
                        lens.isHighlighted = true
                    }
                    guard let item = component.items.first(where: { $0.id == id }) else {
                        return
            ...
            case .cancelled, .failed:
                if let id = self.pressedId, let lens = self.lensViews[id] {
                    lens.performReleaseBounceAnimation()
                    lens.isHighlighted = false
                }
                self.pressedId = nil
            default:
                break
        ...
        private func closestItemId(to point: CGPoint) -> AnyHashable? {
        ...
        func update(component: TabBarComponent, availableSize: CGSize, state: EmptyComponentState, environment: Environment<Empty>, transition: ComponentTransition) -> CGSize {
            ...
            for index in 0 ..< component.items.count {
                ...
                if self.nativeTabBar == nil {
                    let lens: LiquidGlassView
                    if let current = self.lensViews[item.id] {
                        lens = current
                    } else {
                        lens = LiquidGlassView(config: LiquidGlassConfig(tintColor: component.theme.rootController.tabBar.selectedTextColor))
                        lens.isUserInteractionEnabled = false
                        self.lensViews[item.id] = lens
                        if let hostView = itemView.view {
                            self.contextGestureContainerView.insertSubview(lens, belowSubview: hostView)
                        } else {
                            self.contextGestureContainerView.addSubview(lens)
                        }
                    }
                    itemTransition.setFrame(view: lens, frame: itemFrame)
                    lens.updateCornerRadius(itemFrame.height * 0.5)
                    lens.isHighlighted = isItemSelected
                    if let previousComponent, previousComponent.selectedId != item.id, isItemSelected {
                        lens.performReleaseBounceAnimation()
                    }
                }
            ...
            for id in removeIds {
                ...
                self.lensViews.removeValue(forKey: id)
            }
            
            // selectionView and background blur отключены: фон бара прозрачный, линзы — на элементах
